...
### 1b-extra. Cross-channel context (out of scope today)
Other brand folders exist under `~/dev/video-projects/`: `v-aitldr/` (asset-heavy, 0 SRTs as of 2026-05-10), `v-shared/` (Clauding Lab area, project-notes, sync tooling), plus `v-beauty-and-joy/`, `v-kiros/`, `v-supportsignal/`, `v-voz/`. None are in scope today — listed only so we don't accidentally rediscover them later as "new."

### 1b-extra-2. Project manifest staleness
`~/dev/video-projects/v-appydave/projects.json` (2360 lines, top-level `config` + `projects[]` with storage flags) was auto-generated by `dam s3-scan` and last refreshed 2025-12-20 — likely stale. Regen with `dam s3-scan` before relying on it for any "what projects exist" question. FliHub's `/projects` endpoint is the live source of truth; `projects.json` is a snapshot.
