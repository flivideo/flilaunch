...
Write all title candidates to the TITLES section, grouped by hook. Use bold bullet items to label each group — do not use headings or tables.

Also write the audience signal classification as a bullet item (term → Primary/Secondary/Hidden → suggested placement).
...
- **Chapter placeholder**: write `[CHAPTERS — populated by assembling-output]`
...
Note: chapters sub-Delamain runs separately. The CHAPTERS section will be populated when it completes. The description chapter placeholder will be replaced automatically by assembling-output.
