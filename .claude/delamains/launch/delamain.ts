...
export const delamain = defineDelamain({
  ...
  "states": {
    ...
    "awaiting-input": {
      ...
      "phase": "ingest",
      "actor": "operator"
      // Gate 1: operator provides project_code, optional focal_point_note,
      // optional audience_keywords, optional related_videos list.
      // Then advances — agent fetches the transcript.
    },
    ...
    "fetching-transcript": {
      ...
      // Calls FliHub API with project_code, writes transcript to INPUTS section.
    },
    ...
    "running-analysis": {
      ...
      // Fires 12 parallel analysis sub-agents against the transcript.
    ...
    "generating-hooks": {
      ...
      // Each hook tagged with hook_type (x_vs_y, only_one_worked, etc.).
      // Writes to HOOKS section.
    },
    ...
    "awaiting-hook-selection": {
      ...
      "actor": "operator"
      // Gate 2: operator reviews hook angles in HOOKS section,
    ...
    "generating-chapters": {
      ...
      // Reads segment file names (e.g. 01-1-intro.mp4) + SRT timings + transcript
      // from INPUTS. Produces timestamped chapter list with contextualised labels.
    ...
    "awaiting-review": {
      "phase": "review",
      "actor": "operator"
      // Gate 3: operator reviews all generated outputs and:
    ...
    "assembling-output": {
      ...
      // Assembles final copy-ready EXPORT section from all operator selections.
    },
